...
import { Plus, MoreHorizontal, Loader2, GripVertical, Trash2, Zap, Clock } from "lucide-react";
...
export default function TasksPage() {
  ...
  const handleDragEnd = async (result: DropResult) => {
    ...
    const isDoneColumn = destination.droppableId === "done";
                      ...
                      {(provided, snapshot) => (
                        ...
                        >
                          {columnTasks.map((task, index) => {
                            const isOverdue = task.dueAt && isPast(new Date(task.dueAt)) && task.columnId !== "done";
                            const dueLabel = task.dueAt && task.columnId !== "done"
                              ? isOverdue
                                ? `Overdue ${formatDistanceToNow(new Date(task.dueAt), { addSuffix: true })}`
                                  ...
                                  >
                                    <CardHeader className="p-3 pb-2 flex flex-row items-center justify-between space-y-0">
                                      ...
                                      <button
                                        ...
                                        className="text-muted-foreground hover:text-status-urgent transition-colors"
                                      >
                                    ...
                                    <CardContent className="p-3 pt-0 space-y-2">
                                      ...
                                      {dueLabel && (
                                        <div className={`flex items-center gap-1 pl-5 text-[10px] ${isOverdue ? "text-status-urgent" : "text-muted-foreground"}`}>
                                          <Clock className="w-2.5 h-2.5" />
                                          {dueLabel}
                                        </div>
                                      )}
                                      <div className="flex flex-wrap gap-1.5 pl-5">
                                        {task.tags?.filter((t) => !t.startsWith("reminder-")).map((tag) => (
